...
class Payment extends Component {
	state = {
		...
		columns: [{ type: "string" }, { type: "number", label: "Payments" }],
		options: {
			hAxis: {
				type: "number",
				title: "",
	...
	render() {
		const renderedPurchaseHistory = [1, 2, 3, 4, 5, 6].map(function(item) {
			return (
				<ul className="grid grid-3" key={item}>
					<li className="name">
						@Username just purchased 'Criminal Litigation Flashcard'
					</li>
					<li>22-Nov-2018</li>
					<li>20 mins ago</li>
				</ul>
							...
							<div className="top-action">
								...
								<h1>
									...
									{this.state.metrics !== undefined
										? this.state.metrics.total_revenue.toLocaleString()
										: 0}
								</h1>
							</div>
						</li>
						...
						<li>
							<div className="card">
								...
								<div className="total">
									N
									{this.state.metrics !== undefined
										? this.state.metrics.total_revenue_on_flashcards.toLocaleString()
										: 0}
								</div>
							</div>
							<Link to="payments/flashcards">
								Flashcard Purchases <i className="ic-right" />
							</Link>
						</li>
						<li>
							<div className="card">
								...
								<div className="total">
									N
									{this.state.metrics !== undefined
										? this.state.metrics.total_revenue_on_subscriptions.toLocaleString()
										: 0}
								</div>
							</div>
							<Link to="payments/access">
								Flashcard Access <i className="ic-right" />
							</Link>
						</li>
						<li />
					</ul>
			...
			const renderedCourses = store
				...
				.state.courses.map(function(course) {
					...
					return (
						<li key={course.id}>
							<div className="ball">
								<i className={icons[c - 1]} />
							</div>
							<h2 className="title">{course.title}</h2>
							<span>N10,000</span>
							<p>500 Flashcards Purchased</p>
						</li>
					);
				});
			return (
				<div className="main-section">
					<section className="payment-flashcards">
						...
						<div className="test-courses">
							<ul className="grid grid-5">{renderedCourses}</ul>
						</div>
					</section>
				</div>
			);
	...
	render() {
		if (this.state.loadComplete === false) {
			...
			var renderedUsers = this.state.users.map(u => {
				return (
					<ul className="grid grid-4" key={u.id}>
						<li>{u.username}</li>
						<li>
							{this.dateFormat(
								new Date(u.created_at.replace(" ", "T"))
							)}
						</li>
						<li>
							{this.timeSince(
				...
				<div className="main-section">
					<section className="payment-access">
									...
									<div className="head">
										...
										<div>
											<h1>
												{this.state.metrics !==
												undefined
													? this.state.metrics.total_unpaid_users.toLocaleString()
													: 0}
											</h1>
											<p>Users on Free Trials</p>
										</div>
									</div>
								</div>
							</li>
						...
						<div className="table">
							...
							<div className="tbody">{renderedUsers}</div>
						</div>
					</section>
					<Pagination
						config={this.state.pagination}
						load={v => {
