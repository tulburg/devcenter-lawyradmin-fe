...
export class TestView extends Component {
	...
	render() {
		...
		if (this.state.goHome) {
			return <Redirect to={`/dashboard/create-test/${course_id}`} />;
		}
		if (this.state.loadComplete === false) {
			...
			let mappedQuestions = this.state.test.questions.map(function(
				question
			) {
				return (
					<div
						className="question-pane edit-question"
						key={question.serial_no}
						id={`question-${question.serial_no}`}
					>
						<div
							className="question"
							style={
						...
						>
							<ul className="grid grid-2">
								<li className="sparse">ANSWERS</li>
								<li className="sparse" />
							</ul>
						</div>
						<div
				...
				<div className="main-section">
					<section className="test-questions">
						...
						<div className="top-action">
							<select>
								<option value="">
									Year: {this.state.test.year}
								</option>
							</select>
							<button
								className="btn-primary alt sparse"
								onClick={() => {
						...
						{mappedQuestions.length > 0 ? (
							...
							<div className="empty">No question to display</div>
						)}
					</section>
				</div>
			);
...
export default class Test extends Component {
	...
	textAreaChange(q, e) {
		...
		if (q.split("-")[0] === "option_a(desc)") {
			this.values[q.split("-")[1]]["option_a"]["description"] =
				e.target.value;
		}
		...
		if (q.split("-")[0] === "option_b(desc)") {
			this.values[q.split("-")[1]]["option_b"]["description"] =
				e.target.value;
		}
		...
		if (q.split("-")[0] === "option_c(desc)") {
			this.values[q.split("-")[1]]["option_c"]["description"] =
				e.target.value;
		}
		...
		if (q.split("-")[0] === "option_d(desc)") {
			this.values[q.split("-")[1]]["option_d"]["description"] =
				e.target.value;
		}
	}
	fetchQuestions() {
			...
			for (var j = 0; j < tests.length; j++) {
				if (tests[j].id === id) {
					...
					for (var k = 0; k < tests[j].questions.length; k++) {
						this.values[tests[j].questions[k].id] =
							tests[j].questions[k];
					}
				}
			}
	...
	saveQuestion(id) {
		...
		console.log("Working...");
		document.getElementById("overlay-" + question.id).style =
			"display: block;";
		if (question !== undefined) {
			...
			fetch(store.getState().state.api.dev + "questions/" + id, {
				...
				.then(res => res.json())
				.then(res => {
					document.getElementById("overlay-" + res.data.id).style =
						"display: none;";
	...
	linkedQuestion(id) {
		...
		setTimeout(function() {
			...
			var linkLabel = document.getElementById(
				"link-option-" + question.serial_no
			);
			if (linkLabel) {
	...
	chooseLink(id) {
		let question = this.values[id];
		this.values[this.state.linkQuestion.id].linked_to.push(
			question.serial_no.toString()
		);
	}
	setAnswer(value, id) {
			...
			if (tests[j].id === test_id) {
				for (var k = 0; k < tests[j].questions.length; k++) {
					if (tests[j].questions[k].id === undefined) {
						...
						store.dispatch({
							type: "SAVE_COURSE_TESTS_" + course_id,
							payload: tests
						});
					} else {
					}
	...
	render() {
		...
		if (this.state.loadComplete === false) {
			...
			var self = this;
			const mappedLinkOptions = this.state.test.questions.map(function(
				question
			) {
				return question.answer.title !== "" ? (
					<label
						id={`link-option-${question.serial_no}`}
						key={question.serial_no}
						className="link-inputs"
						onClick={e => {
							self.chooseLink(question.id);
						}}
					>
						<input type="checkbox" /> {question.serial_no}
					</label>
				) : (
					""
				);
			});
			const mappedQuestions = this.state.test.questions.map(function(
				question
			) {
				...
				return (
					<div key={question.id} id={`question-${question.id}`}>
						<div className="question-pane">
								...
								<div className="details">
									<div className="action-pane">
										...
										<div className="actions">
											...
											>
												{" "}
												i{" "}
												<div className="dropmenu hint">
													Linking a question enables
													you set multiple follow-up
													questions. This will appear
													in the format - "Use this
									...
									{question.answer.title === "" ? (
										<div
											...
											onClick={() =>
												self.saveQuestion(question.id)
											}
										>
											<button className="sparse">
												SAVE
											</button>
										</div>
									) : (
							...
							>
								<ul className="grid grid-2">
									<li className="sparse">OPTIONS</li>
									<li className="sparse">MORE INFO</li>
								</ul>
							</div>
							<div
								className="options"
						...
						>
							...
							<Select
								classNames=""
								name={"filter" + question.id}
								onChange={v => {
									self.setAnswer(v, question.id);
								}}
								options={[
									{ title: "A", value: "A" },
									{ title: "B", value: "B" },
							...
							<button
								className="sparse"
								onClick={() => self.saveQuestion(question.id)}
							>
								save
							</button>
						</div>
							...
							<div className="linked-modal">
								<div className="question">
									<div className="number">
										{this.state.linkQuestion !== undefined
											? this.state.linkQuestion.serial_no
											: ""}
									</div>
									<div className="details">
										<div className="action-pane">
